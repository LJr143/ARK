<div x-data="{ showFilters: false }" class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
    <div class="px-4 w-full flex justify-end">
        <livewire:dues.compute-dues-payment/>
    </div>
    ...
    <div class="relative z-10 container mx-auto px-4 py-8 space-y-8">
            ...
            <div class="relative flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-gray-600 font-medium mt-1">Manage membership dues efficiently</p>
                    </div>
                </div>
        ...
        <div class="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 p-6">
            <div class="flex flex-col sm:flex-row items-center gap-4">
                <div class="w-full">
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Search Dues</label>
                    <div class="relative group">
                        ...
                        <input wire:model.live="search"
                               type="text"
                               placeholder="Search dues by name, email, or PRC number..."
        ...
        @if (session()->has('message'))
            <div
                class="bg-gradient-to-r from-emerald-50 to-green-50 border-l-4 border-emerald-500 rounded-2xl p-6 shadow-lg mb-6"
                x-data="{ show: true }"
                x-show="show"
        ...
        @if (session()->has('error'))
            <div
                class="bg-gradient-to-r from-red-50 to-rose-50 border-l-4 border-red-500 rounded-2xl p-6 shadow-lg mb-6"
                x-data="{ show: true }"
                x-show="show"
                x-transition:enter="transition ease-out duration-300"
                x-transition:enter-start="opacity-0 transform translate-y-4"
        ...
        <div class="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 overflow-hidden">
                ...
                <table class="w-full divide-y divide-gray-200/50">
                    ...
                    @forelse($dues as $index => $due)
                        <tr class="group hover:bg-gradient-to-r hover:from-blue-50/50 hover:to-indigo-50/50 transition-all duration-300 hover:shadow-md"
                            ...
                            <td class="px-6 py-6">
                                <div class="flex items-center gap-3">
                                    <div
                                        ...
                                        <span
                                            class="text-sm font-medium text-white">{{ substr($due->member->first_name, 0, 1) }}</span>
                                    </div>
                                    <div>
                                        <div
                                            class="font-semibold text-gray-900">{{ trim("{$due->member->first_name} {$due->member->middle_name} {$due->member->family_name}") }}</div>
                                        <div class="text-xs text-gray-500">PRC
                                            No. {{ $due->member->prc_registration_number }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-6">
                                <div class="text-sm text-gray-900">{{ $due->id }}</div>
                            </td>
                            <td class="px-6 py-6 hidden sm:table-cell">
                                <div class="text-sm text-gray-900">{{ $due->fiscalYear->name ?? 'Payment Status: Unpaid' }}</div>
                            </td>
                            <td class="px-6 py-6 hidden xl:table-cell">
                                <div class="text-sm text-gray-900">{{ $due->transaction_reference ?? 'Payment Status: Unpaid' }}</div>
                            </td>
                            <td class="px-6 py-6 hidden xl:table-cell">
                                @if( $due->status === 'paid')
                            ...
                            <td class="px-6 py-6 hidden xl:table-cell">
                                <div
                                    class="text-sm text-gray-700">{{ $due->payment_date ? \Carbon\Carbon::parse($due->payment_date)->format('M j, Y') : \Carbon\Carbon::parse($due->created_at)->format('M j, Y') }}</div>
                                <div
                                    class="text-xs text-gray-500">{{ $due->payment_date ? \Carbon\Carbon::parse($due->payment_date)->format('g:i A') : \Carbon\Carbon::parse($due->created_at)->format('g:i A') }}</div>
                            </td>
                            <td class="px-6 py-6 text-right">
                                <button wire:click="openViewModal({{ $due->id }})"
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <h3 class="text-xl font-bold text-gray-700 mb-2">No dues found</h3>
                                    <p class="text-gray-500 max-w-md">Membership dues will appear here once added</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
            ...
            @if($dues->hasPages())
                <div class="bg-gradient-to-r from-gray-50/50 to-blue-50/50 px-6 py-4 border-t border-gray-200/50">
                    {{ $dues->links('ark.components.pagination.tailwind-pagination') }}
                </div>
            @endif
        </div>
    </div>
            ...
            <div
                ...
                <div
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h3 class="text-2xl font-bold text-gray-900">Due Details</h3>
                            <p class="text-gray-600">{{ trim("{$dueDetails['member']['first_name']} {$dueDetails['member']['middle_name']} {$dueDetails['member']['family_name']}") }}</p>
                        </div>
                    </div>
                ...
                <div class="p-6 space-y-8">
                    <!-- Member Information -->
                    <div
                        class="bg-gradient-to-r from-blue-50 to-purple-50 rounded p-6 border border-blue-200/50">
                            ...
                            <div class="flex-shrink-0 h-14 w-14">
                                <div
                                    class="h-14 w-14 rounded-full bg-gradient-to-r from-indigo-500 to-purple-600 flex items-center justify-center shadow-lg">
                                        <span
                                            class="text-lg font-medium text-white">{{ substr($dueDetails['member']['first_name'], 0, 1) }}</span>
                                </div>
                            </div>
                            <div class="flex-1">
                                <h4 class="text-lg font-semibold text-gray-900">{{ trim("{$dueDetails['member']['first_name']} {$dueDetails['member']['middle_name']} {$dueDetails['member']['family_name']}") }}</h4>
                                <div class="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        <p class="text-sm text-gray-600 font-medium">PRC No.</p>
                                        <p class="text-sm font-semibold text-gray-900">{{ $dueDetails['member']['prc_registration_number'] }}</p>
                                    </div>
                                    <div>
                                        <p class="text-sm text-gray-600 font-medium">Status</p>
                                        <span
                                            class="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold {{ $dueDetails['status'] === 'paid' ? 'bg-emerald-100 text-emerald-800' : ($dueDetails['status'] === 'partial' ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800') }}">
                                                {{ ucfirst($dueDetails['status']) }}
                                            </span>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="space-y-3">
                                <div class="p-4 bg-gray-50/80 rounded-xl">
                                    <label class="block text-sm font-semibold text-gray-700 mb-1">Fiscal
                                        Year</label>
                                    <p class="text-sm font-mono text-gray-900 bg-white px-3 py-2 rounded-lg">{{ $dueDetails['fiscal_year']['name'] ?? 'N/A' }}</p>
                                </div>
                                <div class="p-4 bg-gray-50/80 rounded-xl">
                                    <label class="block text-sm font-semibold text-gray-700 mb-1">Due Amount</label>
                                    <p class="text-sm font-mono text-gray-900 bg-white px-3 py-2 rounded-lg">{{ number_format($dueDetails['amount'], 2) }}</p>
                                </div>
                                <div class="p-4 bg-gray-50/80 rounded-xl">
                                    <label class="block text-sm font-semibold text-gray-700 mb-1">Penalty
                                        Amount</label>
                                    <p class="text-sm font-mono text-gray-900 bg-white px-3 py-2 rounded-lg">{{ number_format($dueDetails['penalty_amount'], 2) }}</p>
                                </div>
                                <div class="p-4 bg-gray-50/80 rounded-xl">
                                    <label class="block text-sm font-semibold text-gray-700 mb-1">Total
                                        Amount</label>
                                    <p class="text-sm font-mono text-gray-900 bg-white px-3 py-2 rounded-lg">{{ number_format($dueDetails['amount'] + $dueDetails['penalty_amount'], 2) }}</p>
                                </div>
                            </div>
                        </div>
                        <div class="space-y-4">
                            ...
                            <div class="space-y-3">
                                <div class="p-4 bg-gray-50/80 rounded-xl">
                                    ...
                                    <p class="text-sm font-mono text-gray-900 bg-white px-3 py-2 rounded-lg">{{ ucfirst($dueDetails['status']) }}</p>
                                </div>
                                <div class="p-4 bg-gray-50/80 rounded-xl">
                                    <label class="block text-sm font-semibold text-gray-700 mb-1">Due Date</label>
                                    <p class="text-sm font-mono text-gray-900 bg-white px-3 py-2 rounded-lg">{{ \Carbon\Carbon::parse($dueDetails['due_date'])->format('F j, Y') }}</p>
                                </div>
                                @if($dueDetails['payment_date'])
                                    <div class="p-4 bg-gray-50/80 rounded-xl">
                                        <label class="block text-sm font-semibold text-gray-700 mb-1">Payment
                                            Date</label>
                                        <p class="text-sm font-mono text-gray-900 bg-white px-3 py-2 rounded-lg">{{ \Carbon\Carbon::parse($dueDetails['payment_date'])->format('F j, Y g:i A') }}</p>
                                    </div>
                                    <div class="p-4 bg-gray-50/80 rounded-xl">
                                        <label class="block text-sm font-semibold text-gray-700 mb-1">Transaction
                                            Reference</label>
                                        <p class="text-sm font-mono text-gray-900 bg-white px-3 py-2 rounded-lg">{{ $dueDetails['transaction_reference'] ?? 'N/A' }}</p>
                                    </div>
                                @endif
                            </div>
                        </div>
